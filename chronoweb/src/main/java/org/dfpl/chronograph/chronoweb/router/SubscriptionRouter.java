...
public class SubscriptionRouter extends BaseRouter {
    ...
    public void registerSubscribeVertexEventRouter(Router router, EventBus eventBus) {
        ...
        router.put("/chronoweb/graph/:time/:kairosProgram/:edgeLabel/:vertexID").handler(routingContext -> {
            ...
            if (existing != null) {
                if (kairosProgram.equals("OutIsAfterReachability") && Server.backendType.equals("memory")) {
                    ...
                    if (gammaTable.getSources().contains(v.getId())) {
                        ...
                        kairos.addSubscription(v, ve.getTime(), edgeLabel, new OutIsAfterReachabilityFile(graph, gammaTable));
                        sendResult(routingContext, 200);
                    }
                } else if (kairosProgram.equals("OutIsAfterReachability") && Server.backendType.equals("persistent")) {
                ...
                if (kairosProgram.equals("OutIsAfterReachability") && Server.backendType.equals("memory")) {
                    ...
                    FixedSizedGammaTable<String, Long> gammaTable;
                    try {
                        gammaTable = OutIsAfterReachabilityFile.createGammaTable(subDirectoryName);
